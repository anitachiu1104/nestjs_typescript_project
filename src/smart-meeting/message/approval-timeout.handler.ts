...
@Injectable()
@DlxMessageDecorator('smart-meeting.approval.timeout')
export class ApprovalTimeoutHandler implements DlxMessageProvider {
    ...
    async handle(message: SignTimeoutMessage, context: RabbitmqContext): Promise<string> {
       ...
       if (booking.state !== 1 || booking.confirm !== 2) {
       ...
       if (mqConsumer.isDelete===0) {
        await context.conn
            .getRepository(SmartMeetingBooking)
            .update({ id: message.id }, { state: 4,releaseType: 5,confirm: 4,updateTime: moment().format(dateFormat)});
